  ...
  return (
    <Flex bgImage={image} bgAttachment={'fixed'} minHeight='100vh' justifyContent='center' justifyItems='center' position={'relative'} top={'5em'}>
        <Flex width='75%' borderRight={'2px solid black'} borderLeft={'2px solid black'} flexDir='column'
            ...
            backdropBlur={'100%'}
            brightness={'-100%'}>
            {items===undefined||items.length===0?<Flex textAlign={'center'} justifyContent={'center'} margin='15px'><Link to={'/'}><Button>Back to shop?</Button></Link></Flex>:<ul>{items.map((e,index)=><CartCard Props={e} key={index}></CartCard>)}</ul>}
