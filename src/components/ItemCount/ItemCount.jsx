...
function Counter({stock, initial, onAdd}) {
    ...
    return (
        <Flex flexDir='column' justifyContent='space-between' justifyItems='space-between' alignItems='center'>
            ...
            <Button width='190px'  bg={'gray.400'} _hover={{bg:'#ffcf00'}} _active={{ bg:'#cc9900'}} transition={'all .5s ease'} onClick={()=>{onAdd()}} margin='10px'>Add to cart<FontAwesomeIcon icon={faCircleArrowRight} style={{position:'relative', top:'1,5px', left:'1.5px'}}/> </Button>
        </Flex>
    )
}
